...
export default function SettingsForm() {
  ...
  return <div className="shell">
    ...
    <main className={styles.page}>
      <header className={styles.intro}><div><p className={styles.eyebrow}>{scope === "operator" ? "Private operator scope" : "Private visitor paper session"}</p><h1>Settings</h1><p>{scope === "operator" ? "Configure the shared executor through its private local channel." : "These settings and trades belong only to this browser session. The shared demo stays untouched."}</p></div><span className={styles.status}>{scope === "visitor" && !ready ? sessionState : run.status}</span></header>
      ...
      <form onSubmit={submit} className={styles.form}>
        ...
        <fieldset className={styles.section} disabled={!ready || locked || busy}><legend>Trading</legend>
          <div className={styles.grid}><label className={styles.field}><span>Network</span><select value={draft.network} onChange={(e) => changeNetwork(e.target.value as TradingSettings["network"])}><option value="testnet">Testnet</option><option value="mainnet">Mainnet</option></select></label><label className={styles.field}><span>Mode</span><select value={draft.mode} disabled={scope === "visitor"} onChange={(e) => change("mode", e.target.value as TradingSettings["mode"])}><option value="paper">Paper</option>{scope === "operator" && <option value="real">Real</option>}</select>{scope === "visitor" && <small>Visitor sessions are paper-only. They never load a wallet or place real orders.</small>}</label><NumberField label="Entry notional, USD" field="quoteUsd" value={draft.quoteUsd} min={0.01} step="any" disabled={locked} onChange={changeNumber} /></div>
        ...
        {scope === "visitor" ? <section className={styles.section}><h2>Connections</h2><p>Visitor paper sessions use official Hyperliquid endpoints. Custom URLs, API keys, authentication headers, and wallet credentials are unavailable so browser sessions cannot reach private trading infrastructure.</p><div className={styles.actions}><button type="button" disabled={!ready || locked || busy} onClick={() => void act(async () => { const result = await context.validateConnection(draft); setMessage(result.message); })}>Check paper connection</button>{connection && <span className={connection.ok ? styles.ok : styles.error}>{connection.message}</span>}</div></section> : <fieldset className={styles.section} disabled={locked || busy}><legend>Connections</legend><p>These venue endpoints do not change this dashboard's Bun API destination.</p>
